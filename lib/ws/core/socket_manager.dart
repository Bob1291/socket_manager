...
class SocketManager {
  ...
  void reqDepth(String symbol, int step, SocketReceiveDataHandler callback) {
  ...
  void reqKline(String symbol, String time, SocketReceiveDataHandler callback,
      {int? endIdx, int? pageSize = 1000}) {
    if (symbol.isEmpty) return;
    WsSendData sendData = WsSendData.req(
      channel: 'market_${symbol}_kline_$time',
      cbId: symbol,
      endIdx: endIdx,
      pageSize: pageSize,
    );
    subscribe(sendData, callback);
  }
