...
import 'package:socket_manager/ws/core/socket_io.dart';
import 'package:socket_manager/ws/core/socket_manager.dart';
...
class RequestSocketManager {
  ...
  connect() async {
    ...
    timer = Timer.periodic(Duration(milliseconds: timerDuration), (timer) {
      reqCache.values.forEach((element) {
        if (_socketIO!.isConnect && element != null) {
          _sendData(element);
        }
      });
    });
  ...
  void _sendData(WsRequest sendData) {
    final appStorage = GetStorage('com.nt.app.cache');
    sendData.token = appStorage.read('token') ?? '';
    sendData.lang = 'zh-cn';
